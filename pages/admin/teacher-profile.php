...
if ($teacherId <= 0) {
    adminFlash('error', 'Invalid teacher profile request.');
...
if (!$teacher) {
    adminFlash('error', 'Teacher not found.');
    header('Location: teachers.php');
    exit;
}

function tf(string $value): string
{
    return htmlspecialchars($value, ENT_QUOTES, 'UTF-8');
}

function teacherFileUrl(string $path): string
{
    if ($path === '') {
        return '';
    }
    if (!preg_match('#^uploads/teacher/[A-Za-z0-9._/-]+$#', $path) || strpos($path, '..') !== false) {
        return '';
    }
...
adminRenderHeader('Teacher Profile', 'teachers');
?>
...
<div class="mb-3">
    <a href="teachers.php" class="btn btn-outline-secondary btn-sm"><i class="fa-solid fa-arrow-left"></i> Back to Teachers</a>
</div>

<div class="card content-card">
    <div class="card-body">
        <div class="d-flex justify-content-between align-items-center flex-wrap gap-2 mb-3">
            <h2 class="h4 fw-semibold mb-0">Teacher Profile</h2>
        ...
        <div class="row g-4">
            <div class="col-lg-4">
                <div class="border rounded-4 p-4 bg-light text-center h-100">
                    <?php $photoUrl = teacherFileUrl((string) ($teacher['photo_path'] ?? '')); ?>
                    <?php if ($photoUrl !== ''): ?>
                        <img src="<?= tf($photoUrl) ?>" alt="Teacher photo" class="rounded-circle border border-3 border-primary-subtle mb-3" style="width:140px;height:140px;object-fit:cover;">
                    <?php else: ?>
            ...
            <?php foreach ($docs as $label => $path): ?>
                <div class="col-md-6 col-xl-3">
                    <div class="border rounded-3 p-3 bg-light h-100">
                        ...
                        <?php $docUrl = teacherFileUrl($path); ?>
                        <?php if ($docUrl !== ''): ?>
                            <a href="<?= tf($docUrl) ?>" target="_blank" class="btn btn-sm btn-outline-primary">Preview</a>
                        <?php else: ?>
