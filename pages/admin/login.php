...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!adminVerifyCsrf($csrf)) {
        ...
        if (!$admin || !password_verify($password, $admin['password'])) {
            ...
            if ($rememberChecked) {
                setcookie('remember_admin_email', $email, time() + (60 * 60 * 24 * 30), '/');
            } else {
...
if ($_SERVER['REQUEST_METHOD'] !== 'POST' && isset($_COOKIE['remember_admin_email'])) {
    $prefillEmail = trim((string) $_COOKIE['remember_admin_email']);
    if ($prefillEmail !== '') {
    ...
    <form method="post" novalidate>
        ...
        <div class="form-group">
            ...
            <div class="input-wrapper">
                ...
                <input type="email" id="email" name="email" value="<?= htmlspecialchars($prefillEmail) ?>" placeholder="admin@email.com" required>
            </div>
        </div>
        <div class="form-group">
